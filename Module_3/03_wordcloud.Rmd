...
```{r importing, eval=FALSE, echo=TRUE}
g1 <-readLines('G1.txt')
...
```{r making_corpora, eval=FALSE, echo=TRUE}
...
```{r to_lower, eval=FALSE, echo=TRUE}
...
```{r numbers, eval=FALSE, echo=TRUE}
...
```{r wordlist, eval=FALSE, echo=TRUE}
g1.m <- as.matrix(g1.dtm)
elpais.m <- as.matrix(elpais.dtm)
